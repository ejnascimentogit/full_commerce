...
**Regra crítica do modo mock — páginas de dados precisam ser Client Components.** `mock/` persiste tudo em `localStorage`, que só existe no navegador. Uma página Next.js que é Server Component roda no processo Node durante o build/request e **não enxerga o `localStorage`** — ela sempre vê os dados de seed estáticos, nunca uma criação/edição feita pelo admin ou pelo próprio cliente (pedido criado no checkout, produto criado no admin, cor da marca alterada em Configurações). Por isso, toda página do storefront/admin que lê dados do `apiClient` em modo mock precisa ser `"use client"` e buscar no `useEffect` — é assim que `app/page.tsx`, `app/catalogo`, `app/produto/[id]`, `app/carrinho`, `app/checkout`, `app/conta*` e `app/pedido/[id]` estão implementadas hoje. Isso deixa de ser necessário quando o modo `rest` estiver ativo (o backend real é acessível tanto do Node quanto do navegador) — mas até lá, um novo Server Component que chama `apiClient` é bug garantido (dado que nunca atualiza). Note também que `apps/storefront` e `apps/admin` rodam em origens diferentes (portas diferentes em dev), então o `localStorage` de um nunca é visto pelo outro — cada app "seed" seus próprios dados iniciais; isso só se resolve de verdade com o backend compartilhado.
...
- **Product**: `id, vendorId, name, description, sku, customerReferenceCode?, categoryId, brand, photos[], unitType (un|kg|cx), basePrice, salePrice?, boxQuantity?, isVariableWeight, avgWeight?, isSeasonal?, stock, variants[], status, weight, dimensions`. `sku` **nunca é digitado pelo admin** — ver **Código do produto (SKU)** abaixo. `customerReferenceCode` (opcional, texto livre) é o código que o próprio cliente já usa pra esse produto no ERP dele; não pode repetir dentro da empresa. `photos` é array ordenado de URLs com uma marcada como capa. `unitType` define como o preço é exibido (`R$/kg`, `R$/un`, `R$/cx`); quando `unitType = "cx"`, `boxQuantity` guarda quantas unidades tem a caixa. `isVariableWeight` marca itens tipo carnes/queijos vendidos por peça mas precificados por kg (`avgWeight` é o peso aproximado exibido, o preço final pode ser ajustado no fornecimento — ver **Preço variável por peso** abaixo). `isSeasonal` é curadoria manual do admin — alimenta a vitrine "Produtos Sazonais" da home (diferente de "mais vendidos", que é calculado dos pedidos, não marcado à mão). `variants[]` (ex: tamanho/cor) cada uma com seu próprio `sku`, `stock` e `priceOverride?` opcional.
- **StoreSettings**: `brandColor, logoUrl?, banners: Banner[], promotionsEnabled, siteCopy, footer, minOrderValue?, freeShippingForCnpj, shippingCost`. Configuração global da loja, editada em **Configurações** no admin — nada aqui é fixo no código, tudo pode ser ajustado pelo dono da loja sem deploy. `brandColor` é uma única cor hex (com atalhos de paleta pré-definida na UI) — os tons claro/hover são derivados automaticamente (`deriveBrandPalette`). `Banner` (`id, imageUrl, title?, linkUrl?, order, active`) alimenta o carrossel da home. `promotionsEnabled` é o freio geral de cupons. `siteCopy` (`storeName, heroTitle, featureBullets[]`) são os textos editáveis da home. `footer` (`legalText, supportEmail?, supportPhone?, paymentMethods: string[], helpLinks: FooterLink[], socialLinks: FooterLink[]`) — `FooterLink` é `{id, label, url}`, tudo listas livres (o admin cadastra quantas formas de pagamento/links/redes quiser, não é uma lista fixa). `minOrderValue` (undefined = sem mínimo) e `freeShippingForCnpj`/`shippingCost` **substituem** as regras antes fixas "sem pedido mínimo" e "frete grátis para CNPJ" do modelo Praso — viraram só o *valor padrão* dessas configurações, não uma regra hardcoded; `calculateShipping(customer, settings)` e a checagem de `minOrderValue` em `createOrder` sempre leem do `StoreSettings` atual.
- **Category** (departamento): `id, name, slug, icon?, parentCategoryId?` — hierárquica, para permitir subcategorias. CRUD só por `platformAdmin` (tela **Departamentos**) — é estrutura global do catálogo, não por fornecedor.
...
### Código do produto (SKU) — sempre gerado pelo sistema

`Product.sku` **nunca é um campo digitável pelo admin**, nem na criação nem na edição — é gerado uma única vez pelo backend e fica fixo depois. A regra é `{número da empresa}{sequencial de 5 dígitos}`: a empresa 1 (fullcommerce) numera seus produtos `100001, 100002, 100003...`; a empresa 2 (o próximo cliente da plataforma) numera os dela `200001, 200002...`; e assim por diante. Cada `Company` tem um `companyNumber` (atribuído na criação da empresa, sequencial: 1, 2, 3...) e um contador próprio de produto — no backend real isso é a função `next_product_code(company_id)` em Postgres, que incrementa o contador da empresa e monta o código.

**Por quê**: antes o admin digitava o SKU livremente, e isso abria a possibilidade real de colar/digitar por engano o código de outro produto (o mesmo risco existia pro `customerReferenceCode`). Gerando pelo sistema, o SKU nunca colide — nem dentro da mesma empresa, nem entre empresas diferentes da plataforma. `customerReferenceCode` continua sendo digitado pelo admin (é o código que o *cliente* usa no ERP dele, não tem como o sistema adivinhar), mas tem uma constraint de unicidade por empresa — tentar salvar um código já usado por outro produto retorna `422 DUPLICATE_REFERENCE_CODE`.

Ao implementar cadastro/edição de produto em qualquer camada (mock, REST, UI), nunca aceitar `sku` como input editável — só exibir o valor já existente (ou "gerado automaticamente" antes de salvar, na criação).

### Gestão de Atividades — quadro de reengajamento de clientes

Módulo (`/atividades` no admin, tela restrita à permissão `atividades`) usado por vendedores/financeiro de cada empresa pra reengajar clientes que pararam de comprar e registrar contatos. Depende do sistema de permissões por usuário/aba (ver **Painel admin** abaixo) — não existe sem ele.

- **ActivityClient**: `id, customerId?, name, phone?, health (green|amber|red), healthReason?, nextContactAt?`. `customerId` é **opcional** — quando preenchido, é um cliente real (linkado à tabela `customers`); quando ausente, é um lead que ainda não comprou, cadastrado na hora pelo vendedor. `health` é o indicador RAG de saúde do relacionamento, editável manualmente.
- **Activity** (o card do kanban): `id, cardNumber, clientId, title, description?, column (urgent|todo|doing|done), priority, createdByAdminId, assignedToAdminId, outcomeId?, imageUrls[], completedAt?`. `cardNumber` é sequencial **por empresa** (mesmo padrão de `orderNumber`/`code` — RPC `next_activity_number`), pra permitir busca futura por número. Mover um card pra coluna `done` **exige** escolher um `outcomeId` antes — não é só uma checagem visual, o backend recusa (`422 OUTCOME_REQUIRED`) se faltar.
- **ActivityOutcome**: `id, name, sortOrder, active`. Lista configurável por empresa (Configurações → Status de conclusão de atividades) do que significa "concluído" — ex: "Convertido em venda", "Cobrança resolvida". Serve tanto vendedor quanto financeiro, sem categoria fixa.
- **Histórico do cliente é derivado, não uma tabela própria**: o painel de cada `ActivityClient` mostra as `Activity` com `column = "done"` e `clientId` igual, ordenadas por `completedAt` — nunca duplicar isso numa tabela de histórico separada.
- **Hierarquia de visibilidade** (quem vê o card de quem): todo `staff` tem um `sectorId` (setor — Financeiro, Televendas etc., cadastrado em Configurações → Equipe) e dois níveis independentes, `isSupervisor` e `isManager`. Regra, do mais restrito ao mais amplo: usuário comum só vê as próprias atividades; `isSupervisor` vê todas as do próprio setor; um `StaffSector.seesAll = true` (ex: Diretoria) ou `isManager = true` em qualquer pessoa (mesmo fora de um setor "vê tudo") faz ela enxergar atividades de **todos** os setores. `isManager` é concedido pessoa a pessoa, não depende de setor — por isso a checagem vem antes do `sectorId` no backend (`visibleAssigneeIds` em `supabase/functions/api/index.ts`). Enforcement é no backend (`GET /admin/activities`), não só escondido no frontend.
- **Painel de desempenho** (`/atividades/desempenho`) agrega, client-side, as atividades concluídas por `assignedToAdminId` e por `outcomeId` — mesmo padrão de agregação já usado em Financeiro (sem endpoint de agregação dedicado no backend).
...
- Cupom (`couponCode`) é validado contra `Promotion.rules` e `maxUses` no momento de aplicar no carrinho, e **revalidado no backend ao criar o `Order`** (nunca confia só no desconto calculado no cliente) — implementado como funções puras (`findPromotionByCoupon`, `calculatePromotionDiscount`, `isPromotionActive`) em `packages/api-client/domain.ts`, reutilizáveis pelo checkout do storefront/mobile e pelo backend real depois.
- Todo cupom passa primeiro pelo freio `StoreSettings.promotionsEnabled` — se desligado, `getPromotionByCoupon` sempre retorna `null` e `createOrder` ignora qualquer `couponCode` enviado, mesmo que a promoção em si esteja com vigência ativa.
- `Promotion.type === "freeShipping"` não gera desconto em R$ (não mexe em `subtotal`) — zera o `shipping` diretamente (`calculateOrderTotals(..., { freeShippingOverride: true })`).

## Painel admin — três papéis, mesma base de código

Por ser multi-fornecedor, `apps/admin` tem visões controladas por `role` no usuário logado (`platformAdmin` | `vendorAdmin` | `staff`), não apps separados — a maior parte da UI (lista de produtos, lista de pedidos) é a mesma tela, só filtrada/restrita conforme o papel:

- **Platform Admin** (a plataforma / você): enxerga tudo — **Fornecedores** (cadastro, ativação, destaque na home), **Roteirização** (cadastra as zonas de entrega e os bairros que cada uma cobre — é isso que resolve o `regionId` de cada cliente), **Departamentos** (CRUD de `Category`, único dono é a plataforma), **Pedidos** de todos os fornecedores, **Painel de Entregas** (view filtrada por `PREPARING`/`OUT_FOR_DELIVERY`, é quem executa a entrega física), **Promoções/Cupons** (globais — `vendorAdmin` também acessa essa tela, mas só cria/vê as com `rules.vendorId` igual ao próprio), **Clientes**, **Financeiro** (extrato de pagamentos e repasse por fornecedor), **Configurações** (cor de marca, logo, banners/carrossel da home, freio geral de promoções, textos editáveis da home, equipe, status de atividades), **Empresas** (se `isPlatformOwner`), **Atividades**.
...
- **Staff** (login de equipe — vendedor, financeiro etc., ver `AdminUser.permissions`): só acessa as abas explicitamente marcadas pelo `platformAdmin` da própria empresa, dentre `produtos, pedidos, clientes, financeiro, promocoes, departamentos, fornecedores, atividades`. **Empresas e Configurações nunca são concedíveis** a esse papel, sem exceção — nem via checkbox nem editando o array direto no banco, o backend (`requirePermission`) recusa. Criado em Configurações → Equipe, cada `staff` loga com a própria conta (não é nome digitado num campo) — é assim que o sistema sabe quem fez cada ação (ex: quem atendeu um card de **Atividades**).

Todo endpoint `admin/*` do contrato de API deve aceitar essa distinção — filtrar por `vendorId` do token quando o papel for `vendorAdmin`, e checar `requirePermission` quando o papel for `staff`, é responsabilidade do backend, não do frontend.

O dono da loja cria sua própria conta `platformAdmin` em `/criar-conta` no admin (autocadastro, igual ao cadastro do cliente na loja) — não depende das contas demo do seed. Num backend real isso deve virar convite/setup único (não autocadastro aberto para sempre), mas nesta fase de mock fica simples de propósito.
