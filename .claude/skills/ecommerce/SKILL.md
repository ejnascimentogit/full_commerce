...
- **StoreSettings**: `brandColor, logoUrl?, banners: Banner[], promotionsEnabled, siteCopy`. Configuração global da loja, editada em **Configurações** no admin. `brandColor` é uma única cor hex — os tons claro/hover são derivados automaticamente (`deriveBrandPalette`). `Banner` (`id, imageUrl, title?, linkUrl?, order, active`) alimenta o carrossel da home. `promotionsEnabled` é o freio geral: desligado, nenhum cupom aplica desconto no checkout mesmo com `Promotion` ativa — não precisa apagar as promoções, só liga/desliga. `siteCopy` (`storeName, heroTitle, featureBullets[]`) são os textos editáveis da home — dá ao admin autonomia sobre nome/frases sem precisar de deploy; storefront lê isso em vez de string fixa.
