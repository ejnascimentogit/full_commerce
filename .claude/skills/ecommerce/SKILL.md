...
- **StoreSettings**: `brandColor, logoUrl?, banners: Banner[], promotionsEnabled, siteCopy, footer`. Configuração global da loja, editada em **Configurações** no admin. `brandColor` é uma única cor hex — os tons claro/hover são derivados automaticamente (`deriveBrandPalette`). `Banner` (`id, imageUrl, title?, linkUrl?, order, active`) alimenta o carrossel da home. `promotionsEnabled` é o freio geral: desligado, nenhum cupom aplica desconto no checkout mesmo com `Promotion` ativa — não precisa apagar as promoções, só liga/desliga. `siteCopy` (`storeName, heroTitle, featureBullets[]`) são os textos editáveis da home. `footer` (`legalText, supportEmail?, supportPhone?, socialLinks`) alimenta o rodapé global do storefront (razão social/CNPJ/endereço, contato, redes sociais) — mesma ideia: sem precisar de deploy para o admin trocar.
