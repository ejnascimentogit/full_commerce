...
- **StoreSettings**: `brandColor, logoUrl?, banners: Banner[]`. Configuração global da aparência da loja, editada em **Configurações** no admin. `brandColor` é uma única cor hex — os tons claro/hover são derivados automaticamente (`deriveBrandPalette`), então a tela de admin só precisa de um color picker. `Banner` (`id, imageUrl, title?, linkUrl?, order, active`) alimenta o carrossel da home.
